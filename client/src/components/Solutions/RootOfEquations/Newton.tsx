...
import {
  ...
  NewTonResponse,
  NewTonRequest
} from "../../../Methods/rootMethods/newton";
...
function Newton() {
  ...
  const [Data, setData] = useState<NewTonResponse | null>(null);
    ...
    if (Id != null) {
      new Promise((resolve) => {
        ...
        .then((result: unknown) => {
          ...
          const input = data.input as NewTonRequest;
  ...
  const sendRequest = async () => {
    ...
    new Promise((resolve) => {
      setTimeout(() => {
        resolve(NewTonMethod(xInitial, equation, errorfactor));
      }, 1000);
    })
      .then((result: unknown) => {
        const graphicalResponse = result as NewTonResponse;
  ...
  return (
    <div className="w-full mt-5">
      ...
      <div className="w-full flex flex-col my-10">
        ...
        <Table data={Data as NewTonResponse} />
      </div>
    </div>
  );
}

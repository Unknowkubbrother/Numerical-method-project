...
import {
  TrapezoidalMethods,
  TrapezoidalResponse,
} from "../../../Methods/integration/Trapezoidal";
...
function Trapezoidal() {
  ...
  const renderSolution = () => {
    return (
      <div className="w-[95%] flex justify-center items-center flex-col p-5">
        ...
        <div className="flex flex-col justify-start items-start m-auto flex-wrap">
          <div className="flex justify-center items-center gap-5">
            ...
            />
            <BlockMath
              math={`\\int_{${Result?.result.x0.x}}^{${Result?.result.x1.x}} ${Result?.result.equation} \\kern{5px} dx`}
            />
          </div>
          <div className="flex justify-center items-start gap-5">
            ...
            <div className="flex flex-col justify-start items-start">
              <BlockMath
                math={`x_{0} \\kern{3px} = a = \\kern{3px}${Result?.result.x0.x}; \\kern{3px} f(x_{0}) = \\kern{3px} ${Result?.result.x0.fx} \\kern{3px} = \\kern{3px} ${Result?.result.x0.ans}`}
              />
              <BlockMath
                math={`x_{1} \\kern{3px} = b = \\kern{3px}${Result?.result.x1.x}; \\kern{3px} f(x_{1}) = \\kern{3px} ${Result?.result.x1.fx} \\kern{3px} = \\kern{3px} ${Result?.result.x1.ans}`}
              />
            </div>
          </div>
          <div className="flex justify-center items-center gap-5">
            <BlockMath math="Thus" />
            <BlockMath
              math={`I \\kern{3px} = \\kern{3px} \\frac{h}{2}\\kern{5px}[f(x_{0})+f(x_{1})] \\kern{3px} = \\kern{3px}
